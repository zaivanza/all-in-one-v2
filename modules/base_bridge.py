...
class BaseBridge:
    ...
    def __init__(self, key, number, params=None):
        ...
        if self.params:
            ...
            self.amount_from = Value_BaseBridge.amount_from
            self.amount_to = Value_BaseBridge.amount_to
            self.bridge_all_balance = Value_BaseBridge.bridge_all_balance
            self.min_amount_bridge = Value_BaseBridge.min_amount_bridge
            self.keep_value_from = Value_BaseBridge.keep_value_from
            self.keep_value_to = Value_BaseBridge.keep_value_to
    ...
    async def get_txn(self):
        ...
        try:
            contract = self.manager.web3.eth.contract(address=Web3.to_checksum_address("0x49048044D57e1C92A77f79988d21Fa8fAF74E97e"), abi=ABI_BASE_BRIDGE)
            contract_txn = await contract.functions.depositTransaction(
